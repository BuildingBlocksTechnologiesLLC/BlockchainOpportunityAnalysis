...
def nlp_body(text,tagger,crawlDay, trimmed):
    """
...
#Date Function
    re_date = re.compile(r'(?i) date post[:| ]|date posted[:| ]| posted[:| ]| post[:| ] | posted on[:| ]') 
    #find date text matched of dates found
    re_text = re_text + trimmed
...
def index_json(file_path,stanfordnlp,ner):
    ...
    with open(file_path,"r") as json_file:
        ...
        if True :
            ...
            text_loc, text_org,text_sal, text_date, b_city,remote = nlp_body(job['Body'],tagger,job['CrawledDate'],job['Trimmed'])
