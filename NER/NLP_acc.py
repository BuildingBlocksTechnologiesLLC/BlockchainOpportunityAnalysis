...
' PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY')
            ...
            if title_loc.strip() == correct_loc.strip() or text_loc.strip() == correct_loc.strip():
                total_cor_loc += 1 
            #else:
            #    incorrect = incorrect+f+'\n'+ 'correct: ' +correct_loc +'\n' +'pred: '+text_loc+' - '+title_loc+'\n'
            if location.strip() == correct_loc.strip():
                tl += 1
            
            if org.strip() == correct_org.strip():
                to += 1
            ...
            with open('C:/Users/Leon/BlockchainOpportunityAnalysis/NER/predictions/'+f[:-3]+'.txt', "w") as w:
                w.write(json_str)
    ...
    print('Total location Accuracy:',tl/tot)
    print('Total company Accuracy:',to/tot)
    ...
    with open('locincorrect.txt','w') as o:
        o.write(incorrect)
'''
Title location Accuracy: 0.7288503253796096
Body location Accuracy: 0.36225596529284165
Title company Accuracy: 0.913232104121475
Body company Accuracy: 0.1648590021691974
...
if __name__ == "__main__":
    ...
    print(datetime.now())
#Total location Accuracy: 0.8351409978308026